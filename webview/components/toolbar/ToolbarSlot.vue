            ...
            >
                ...
                <div v-else class="h-full w-full">
                    <div class="relative h-full w-full p-1">
                        <img :src="item.icon" :alt="item.name" :class="imageClasses" draggable="false" />
...
const slotClasses = computed(() => ({
    ...
    'group-hover:border-white/20': !props.item,
}));

const imageClasses = computed(() => ({
    'h-full w-full rounded-lg object-cover transition-transform duration-300': true,
    'group-hover:scale-110': isHovered.value,
}));
